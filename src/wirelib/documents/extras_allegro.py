...
def export_allegro():
    ...
    filename = u"WiReLib", datetime.strftime("%d-%m-%Y", date) + u".ADT"
...
def __doc_to_string(document):
    ...
    line_end = u"\n"
    doc_str = u"" + allegro_dict[u"isbn"], document.isbn + line_end
    doc_str += allegro_dict[u"publisher"], document.publisher.name + line_end
    try:
        vol = extra_fields.get(bib_field__iexact="volume")
    except doc_extra.DoesNotExist:
        vol = u""
    doc_str += allegro_dict[u"volume"], vol + line_end
    try:
        series = extra_fields.get(bib_field__iexact="series")
    except doc_extra.DoesNotExist:
        series = u""
    doc_str += allegro_dict[u"series"], series + line_end
    ...
    if len(authors) > 1:
        ...
        for auth_i in authors[1:]:
            ...
            j += 1
    doc_str += (allegro_dict[u"LibraryOfCongressNo"], document.lib_of_con_nr
            + line_end)
    doc_str += allegro_dict[u"bookid"], document.bibtex_id + line_end
    doc_str += allegro_dict[u"title"], document.title + line_end
    doc_str += allegro_dict[u"address"], document.address + line_end
    """
    ...
    doc_str += allegro_dict[u"keywords"], keywords + line_end
    doc_str += allegro_dict[u"year"], document.pub_date.year + line_end
    return doc_str
